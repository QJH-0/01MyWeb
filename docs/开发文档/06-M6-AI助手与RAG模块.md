...
- 引用来源展示：每条回答可展示引用列表（title/url），支持跳转到站内项目/博客。
- 异常与降级：后端/AI 服务不可用时提供占位对话或不可用视图，并提示“后端连接失败/AI 服务不可用”；发送消息明确失败，不误报成功。
- 认证交互：未登录访问引导登录；登录后可正常发起 SSE。

### 后端开发计划（M6）

- 会话与消息持久化：`ai_session`、`ai_message`（含索引与软删字段），按 `userId + sessionId` 隔离。
- SSE 对话接口：实现 `GET /api/ai/chat/stream`，事件名严格限定 `delta/done/error`（契约以 `API文档与系统架构.md` 为准）。
- 会话管理接口：`GET /api/ai/sessions`、`GET /api/ai/sessions/{sessionId}/messages`、`DELETE /api/ai/sessions/{sessionId}`（登录必需）。
- 引用链路：结合站内检索结果构建 sources，并在 `done` 事件携带（最小字段 `title/url/sourceType/sourceId`）。
- 稳定性：空问题校验、超时控制、限流与失败降级（可解释错误码 + `error` 事件）。
- 自动化：覆盖 SSE 事件序列、会话隔离（404 不泄露）、空问题、异常分支与引用结构。

### 联调与验收里程碑（M6）

- 里程碑 1：SSE 流式展示可用（delta -> done/error），前端不白屏不串台。
- 里程碑 2：会话列表/消息历史/删除闭环可用，引用可展示与跳转。
- 里程碑 3：构建与测试证据回填到 `99-阶段验收清单.md` 的 `M6` 区域，完成阶段闸门 A~E。
