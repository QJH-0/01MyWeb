...
- **后端**：单一 **Spring Boot 3.4.x**（Java 21）进程提供业务 API、搜索编排、文件服务及 **AI 能力**；AI 非独立微服务，而是在应用内通过 **Spring AI Alibaba 1.1.2.2** 调用阿里云 DashScope（通义千问），并与 Elasticsearch、Redis、MySQL、MinIO 协同。
...
flowchart TB
  User[用户] --> Nginx[Nginx]
  Nginx --> Static[前端静态资源]
  Nginx --> Boot[Spring Boot 3.4]
  Boot --> MySQL[(MySQL 8)]
  Boot --> ES[(Elasticsearch 8)]
  Boot --> Redis[(Redis 7)]
  Boot --> MinIO[(MinIO)]
...
| 层级 | 技术/工具 | 版本/说明 |
|------|----------|----------|
| **前端** | Vue | 3.5.x（Composition API，随当前稳定版） |
| | UI | 自定义为主 + Element Plus 2.x |
| | 样式 | SCSS + CSS 变量 |
| | 动画 | GSAP 3.x + Vue 内置过渡 |
| | HTTP | Axios |
| | 构建 | Vite 6.x |
| **后端** | Java | 21 (LTS) |
| | Spring Boot | **3.4.x**（使用当前 3.4 线最新补丁版） |
...
| | Elasticsearch | 服务端 8.x；Java API 客户端主版本与集群一致 |
...
| | MySQL | Connector 随 Spring Boot 管理或与 8.0 实例匹配 |
| | Redis | Lettuce（Spring Boot 默认） |
...
- **nginx**：`nginx:alpine`，暴露 80/443，挂载配置与前端 `dist`。  
- **app**：后端镜像，依赖 MySQL、ES、Redis、MinIO；`SPRING_PROFILES_ACTIVE` 区分环境。  
- **mysql**：`mysql:8.0`，库名与账号由环境变量注入。  
- **elasticsearch**：镜像主版本与 Java 客户端一致；单节点 discovery 与环境变量按 ES 8 官方文档；**开发可临时关闭安全插件简化联调，生产必须启用认证与 TLS**。  
- **redis**：`redis:7-alpine`；生产建议密码与持久化策略。  
- **minio**：官方镜像固定 tag；控制台端口单独映射；数据目录 **必须挂卷**。  
...
- 以 **Spring Boot 3.4.x** 的依赖管理为主；额外库（ES Java API、MinIO、Spring AI Alibaba）版本与 BOM 冲突时 **以 Spring Boot 与 Spring AI Alibaba 官方兼容矩阵为准**。  
